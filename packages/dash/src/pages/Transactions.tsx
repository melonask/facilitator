...
export function Transactions() {
  ...
  const [filterFacilitator, setFilterFacilitator] = useState<string | null>(
    null,
  );
  ...
  const filteredTxs = useMemo(() => {
    ...
    if (filterFacilitator) {
      txs = txs.filter(
        (t) => t.from.toLowerCase() === filterFacilitator.toLowerCase(),
      );
    }
  ...
  const stats = useMemo(() => {
    const totalValue = filteredTxs.reduce(
      (sum, tx) => sum + BigInt(tx.value),
      0n,
    );
    const totalGas = filteredTxs.reduce(
      (sum, tx) => sum + BigInt(tx.gasCost || 0),
      0n,
    );
    const totalTokens = filteredTxs.reduce(
      (sum, tx) => sum + tx.tokensTransferred.length,
      0,
    );
    ...
    for (let i = hoursToShow - 1; i >= 0; i--) {
      ...
      const count = filteredTxs.filter(
        (tx) => tx.timestamp >= hourStart && tx.timestamp < hourEnd,
      ).length;
    ...
    const avgGas =
      filteredTxs.length > 0 ? totalGas / BigInt(filteredTxs.length) : 0n;

    // Average value per transaction
    const avgValue =
      filteredTxs.length > 0 ? totalValue / BigInt(filteredTxs.length) : 0n;

    // Unique senders and receivers
    const uniqueSenders = new Set(
      filteredTxs.map((tx) => tx.from.toLowerCase()),
    ).size;
    const uniqueReceivers = new Set(
      filteredTxs.filter((tx) => tx.to).map((tx) => tx.to!.toLowerCase()),
    ).size;
  ...
  const periodComparison = useMemo(() => {
    ...
    const prevTxs = transactions.filter(
      (t) => t.timestamp >= prevStart && t.timestamp < startTime,
    );
    ...
    const currentGas = filteredTxs.reduce(
      (sum, tx) => sum + BigInt(tx.gasCost || 0),
      0n,
    );
    const prevGas = prevTxs.reduce(
      (sum, tx) => sum + BigInt(tx.gasCost || 0),
      0n,
    );
  ...
  const getCurrency = (tx: Transaction) =>
    networks.find((n) => n.id === tx.networkId)?.currency || "ETH";
  const getNetworkName = (tx: Transaction) =>
    networks.find((n) => n.id === tx.networkId)?.name || "Unknown";
  const getFacilitatorLabel = (address: string, networkId: string) => {
    const f = facilitators.find(
      (f) =>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                {periodComparison && (
                  <TrendIndicator
                    value={periodComparison.countChange}
                    className="text-[10px]"
                  />
                )}
              </div>
              <p className="text-[10px] text-muted-foreground mt-0.5">
                {stats.uniqueSenders} unique sender
                {stats.uniqueSenders !== 1 ? "s" : ""}
              </p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold mt-1">
                {parseFloat(formatEther(stats.totalValue)).toFixed(4)}
              </p>
              <p className="text-[10px] text-muted-foreground mt-0.5">
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold mt-1">
                {parseFloat(formatEther(stats.totalGas)).toFixed(4)}
              </p>
              <p className="text-[10px] text-muted-foreground mt-0.5">
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-[10px] text-muted-foreground mt-0.5">
                {stats.uniqueReceivers} recipient
                {stats.uniqueReceivers !== 1 ? "s" : ""}
              </p>
            </div>
        ...
        <div className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-3 sm:gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <select
              ...
              value={filterNetwork || ""}
              onChange={(e) =>
                handleFilterChange(setFilterNetwork, e.target.value || null)
              }
            >
            ...
            <select
              ...
              onChange={(e) =>
                handleFilterChange(setFilterFacilitator, e.target.value || null)
              }
            >
          ...
          <div className="divide-y">
            ...
            {paginatedTxs.length === 0 ? (
              ...
              paginatedTxs.map((tx) => {
                ...
                const gasVal = tx.gasCost
                  ? parseFloat(formatEther(BigInt(tx.gasCost))).toFixed(6)
                  : "-";
                  ...
                  >
                    ...
                    <div className="md:hidden space-y-2">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <div className="text-right shrink-0">
                          <p
                            className={`font-mono text-xs font-medium ${isZeroVal ? "text-muted-foreground" : ""}`}
                          >
                            {val} {net?.currency}
                          </p>
                          <p className="text-[10px] text-muted-foreground">
                      ...
                      <div className="flex items-center justify-between gap-2 text-xs">
                        <div className="flex items-center gap-1.5 min-w-0">
                          <span className="text-muted-foreground truncate">
                            {getFacilitatorLabel(tx.from, tx.networkId)}
                          </span>
                          <ArrowRight
                            size={10}
                          ...
                          <span className="font-mono truncate">
                            {tx.to ? tx.to.slice(0, 8) + "..." : "Contract"}
                          </span>
                        </div>
                    ...
                    <div className="hidden md:grid grid-cols-12 gap-4 items-center">
                      ...
                      <div className="col-span-2 flex items-center gap-2">
                        <Badge
                          variant="outline"
                          className="text-[10px] h-5 truncate"
                        >
                          {net?.name}
                        </Badge>
                        {tx.tokensTransferred.length > 0 && (
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between p-3 border-t bg-muted/30">
              <p className="text-xs text-muted-foreground">
                ...
                {Math.min(currentPage * ITEMS_PER_PAGE, filteredTxs.length)} of{" "}
                {filteredTxs.length}
              </p>
