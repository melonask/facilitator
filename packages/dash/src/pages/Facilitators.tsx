...
import { HealthScore, ProgressBar } from "@/components/ui/progress-bar";
...
export function Facilitators() {
  ...
  const sortedFacilitators = useMemo(() => {
    ...
    list.sort((a, b) => {
      const keyA = `${a.networkId}:${a.id.toLowerCase()}`;
      const keyB = `${b.networkId}:${b.id.toLowerCase()}`;
      const sA = stats[keyA];
      const sB = stats[keyB];
      ...
      switch (sortBy) {
        ...
        case "tx-asc":
          return txA - txB;
        case "gas-desc":
          return gasA > gasB ? -1 : gasA < gasB ? 1 : 0;
        case "name":
  ...
  const recentActivityByFacilitator = useMemo(() => {
    ...
    const activityMap: Record<
      string,
      { count: number; lastTx: number | null }
    > = {};
      ...
      <DialogContent className="sm:max-w-2xl w-[calc(100vw-2rem)]">
        <DialogHeader>
          ...
          <DialogDescription className="flex items-center gap-2 flex-wrap">
            ...
            <span className="font-mono text-xs truncate max-w-[180px] sm:max-w-[280px]">
              {f.id}
            </span>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            ...
            <Card className="p-2 text-center">
              ...
              <p className="text-lg font-bold">
                {parseFloat(formatEther(gasSpent)).toFixed(4)}
              </p>
              <p className="text-[10px] text-muted-foreground">
                {net?.currency}
              </p>
                  ...
                  >
                    <defs>
                      ...
                      >
                        <stop
                          offset="0%"
                          stopColor={getNetworkColor(f.networkId)}
                          stopOpacity={0.5}
                        />
                        <stop
                          offset="50%"
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-xs sm:text-sm mt-1">
            ...
            {facilitators.length !== 1 ? "s" : ""} across {networks.length}{" "}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Tabs
            value={viewMode}
            onValueChange={(v) => setViewMode(v as "grid" | "compact")}
          >
            <TabsList className="h-8">
              <TabsTrigger value="grid" className="text-xs px-2">
                Grid
              </TabsTrigger>
              <TabsTrigger value="compact" className="text-xs px-2">
                Compact
              </TabsTrigger>
            </TabsList>
          </Tabs>
              ...
              <p className="text-lg font-bold">
                {facilitators
                  .reduce((sum, f) => {
                    ...
                    const s = stats[key];
                    return (
                      sum + (s ? parseFloat(formatEther(BigInt(s.balance))) : 0)
                    );
                  }, 0)
                  .toFixed(4)}
              </p>
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-lg font-bold">
                {parseFloat(
                  formatEther(
                    Object.values(gasSpentByFacilitator).reduce(
                      (sum, g) => sum + g,
                      0n,
                    ),
                  ),
              ...
              <p className="text-lg font-bold">
                {
                  facilitators.filter((f) => {
                    ...
                    const s = stats[key];
                    const balance = s
                      ? parseFloat(formatEther(BigInt(s.balance)))
                      : 0;
                    const history =
                      s?.history
                        ?.slice(-20)
                        .map((h) =>
                          parseFloat(formatEther(BigInt(h.balance))),
                        ) || [];
                    return (
                      getFacilitatorHealth(balance, s?.txCount || 0, history) >=
                      70
                    );
                  }).length
                }
              ...
              <p className="text-lg font-bold">
                {
                  facilitators.filter((f) => {
                    ...
                    const s = stats[key];
                    return (
                      s && parseFloat(formatEther(BigInt(s.balance))) < 0.1
                    );
                  }).length
                }
              </p>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {sortedFacilitators.map((f) => {
            ...
            const balanceHistory =
              s?.history
                ?.slice(-30)
                .map((h) => parseFloat(formatEther(BigInt(h.balance)))) || [];
            ...
            const healthScore = getFacilitatorHealth(
              balance,
              txCount,
              balanceHistory,
            );
            ...
            const networkColor = getNetworkColor(f.networkId);

            return (
              <Card
                key={key}
                className={`overflow-hidden transition-all hover:shadow-sm ${isLowBalance ? "border-destructive/50" : ""}`}
              >
                  ...
                  <div className="space-y-1.5 min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {f.privateKey && (
                        <Badge
                          variant="outline"
                          className="text-[10px] h-5 px-1.5 gap-1 font-normal text-muted-foreground shrink-0"
                        >
                          <HugeiconsIcon
                            icon={LockIcon}
                            className="h-3 w-3"
                  ...
                  <div>
                    <div className="flex items-baseline justify-between mb-2">
                      <div className="flex items-baseline gap-2">
                        <span
                          className={`font-mono text-2xl font-bold tracking-tight ${isLowBalance ? "text-destructive" : ""}`}
                        >
                          {balance.toFixed(4)}
                        </span>
                      ...
                      {balanceTrend !== 0 && (
                        <TrendIndicator value={balanceTrend} />
                      )}
                    </div>
                    <ProgressBar
                      value={balance}
          ...
          {sortedFacilitators.length === 0 && (
            <div className="col-span-full text-center text-muted-foreground py-12">
              {filterNetwork
                ? "No facilitators on this network."
                : "No facilitators configured. Go to Settings to add one."}
            </div>
          )}
        </div>
      ) : (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge
                          variant="outline"
                          className="text-[10px] h-4 hidden sm:inline-flex"
                        >
                          {net?.name}
                        </Badge>
                        {f.privateKey && (
                    ...
                    <div className="flex items-center gap-3 sm:gap-6 text-right">
                      <div>
                        ...
                        <p
                          className={`text-xs sm:text-sm font-mono font-semibold ${isLowBalance ? "text-destructive" : ""}`}
                        >
                          {balance.toFixed(4)}
                        </p>
                      </div>
