...
async function main() {
    ...
    const deployUsdcHash = await walletClient.deployContract({
        ...
        bytecode: eip3009Artifact.bytecode.object,
    });
    const usdcAddress = (await publicClient.waitForTransactionReceipt({ hash: deployUsdcHash })).contractAddress;
    ...
    detail("buyer", `${buyer.address.slice(0, 6)}.. ${CYAN}1000 USDT + 1000 USDC${RESET}`);
