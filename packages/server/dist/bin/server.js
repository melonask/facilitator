...
import { Eip7702Mechanism, KNOWN_DELEGATE_ADDRESSES, } from "@facilitator/eip7702";
...
async function main() {
    ...
    const addConfig = async (id, rpcs) => {
        provider.addChain(id, rpcs);
        // Resolve delegate address
        let delegate = args["delegate-address"] ?? KNOWN_DELEGATE_ADDRESSES[id];
        if (!delegate) {
    ...
    // --- Register Mechanisms ---
    const globalDelegateOverride = args["delegate-address"];
    ...
    facilitator.register(registeredChains.map(id => `eip155:${id}`), eip7702);
    ...
    for (const chainId of registeredChains) {
        ...
        facilitator.register([`eip155:${chainId}`], erc3009);
    }
    // --- Server ---
